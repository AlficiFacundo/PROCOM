...
def build_frame(device, data=b''):
    #Recibe el device y la data a enviar.
    size = len(data)
    is_long = size > 15
    ...
    b3 = device
    return bytes([b0, b1, b2, b3]) + data + bytes([trailer]) #retornar trama completa


def parse_response(ser):
    header = ser.read(4)
    if len(header) < 4:
        return None, None #verificación del header ---> si no se reciben 4 bytes retorna none

    b0 = header[0]
    if (b0 >> 5) != 0b101: #check de inicio de trama
        return None, None

    is_long = (b0 >> 4) & 0x01 #guardo bit L/S
    device = header[3]
    size = ((header[1] << 8) | header[2]) if is_long else (b0 & 0x0F) #calculo el tamaño de la data si es larga.
