...
// los mismos campos leidos en la cabecera. Devuelve -1 si no matchea.
static int read_trailer(Header *h){
    u8 t;
    read_bytes(&t,1);

    if(((t>>5)&0x07) != 0x02){   // 010
        return -1;
    }
...
static void build_and_send_frame(u8 device, u8 *data, u16 size){
    ...
    if(is_long){
        ...
        hdr[0] = (0x05<<5) | (size&0x0F);
        hdr[1] = 0x00;
        hdr[2] = 0x00;
        trl[0] = (0x02<<5) | (size&0x0F);
    }
    hdr[3] = device;
...
int main()
{
    ...
    while(1){
        ...
        if(h.size>0 && h.size<=DATA_BUF_MAX){
            read_bytes(data,h.size);
        }

        if(read_trailer(&h)!=0){
            continue; // trailer no matchea con la cabecera, se descarta la trama
        }
        ...
        switch(h.device){
            ...
            case DEV_LED3: {
                int led = h.device;
                u8 color = (h.size>0)? data[0] : 0x00;
                u32 mask  = 0x07u << (led*3);
                u32 value = (u32)(color & 0x07) << (led*3);
