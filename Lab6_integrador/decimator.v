module decimator #(
    ...
    reg [$clog2(N_PHASES)-1:0] phase_sw_reg;

    always @(posedge clk or negedge i_rst_n) begin
        if (!i_rst_n)
            phase_sw_reg <= {$clog2(N_PHASES){1'b0}};
        else if (phase_cnt == N_PHASES-1)
            phase_sw_reg <= phase_sw;
    end

    wire match = en_tx && en_rx && (phase_cnt == phase_sw_reg);
